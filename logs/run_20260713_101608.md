...
print('device:', _DEV, '(switch Runtime->GPU if this says cpu)')
_cache = {}   # load each model ONCE, not once per frame

def infer_depth_pro(img_rgb):
    import depth_pro
    if 'depth_pro' not in _cache:
        if not os.path.exists('checkpoints/depth_pro.pt'):   # weights are a separate download
            os.makedirs('checkpoints', exist_ok=True)
            os.system('wget -q https://ml-site.cdn-apple.com/models/depth-pro/depth_pro.pt -P checkpoints')
        m, tf = depth_pro.create_model_and_transforms(device=torch.device(_DEV))
        _cache['depth_pro'] = (m.eval(), tf)
    m, tf = _cache['depth_pro']
    with torch.no_grad():
        pred = m.infer(tf(img_rgb), f_px=f)
    return pred['depth'].squeeze().float().cpu().numpy().astype('float32')   # metric metres

def infer_unidepth_v2(img_rgb):
    from unidepth.models import UniDepthV2
    if 'unidepth' not in _cache:
        _cache['unidepth'] = UniDepthV2.from_pretrained('lpiccinelli/unidepth-v2-vitl14').to(_DEV).eval()
    model = _cache['unidepth']
    rgb = torch.from_numpy(img_rgb).permute(2,0,1).to(_DEV)
    K = torch.tensor([[f,0,cx],[0,f,cy],[0,0,1]], dtype=torch.float32).to(_DEV)
    with torch.no_grad():
        pred = model.infer(rgb, K)
    return pred['depth'].squeeze().float().cpu().numpy().astype('float32')   # metric z, metres

def infer_metric3d_v2(img_rgb):
    # Metric3D needs its canonical-camera resize + intrinsic scaling before it is metric.
...
**output:**
```text
device: cuda (switch Runtime->GPU if this says cpu)
```
...
**output:**
```text
depth_pro      {'error': "AttributeError: 'float' object has no attribute 'squeeze'"}
unidepth_v2    {'error': "ModuleNotFoundError: No module named 'unidepth'"}
metric3d_v2    {'error': 'NotImplementedError: port Metric3D canonical transform before use'}
```
...
# Echo the key results into the log so the number is captured even if it ran earlier.
print('=== results snapshot ===')
print('results:', globals().get('results', '(section 6 not run yet)'))
...
```text
=== results snapshot ===
results: {'depth_pro': {'error': "AttributeError: 'float' object has no attribute 'squeeze'"}, 'unidepth_v2': {'error': "ModuleNotFoundError: No module named 'unidepth'"}, 'metric3d_v2': {'error': 'NotImplementedError: port Metric3D canonical transform before use'}}
